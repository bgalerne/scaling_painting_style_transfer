# This is an adaptation of the main module spst.py to perform ultra-high resolution texture synthesis
# Reference:
...
# Parse arguments:
import argparse
parser = argparse.ArgumentParser(description="Texture synthesis using Scaling Painting Style Transfer: High-quality style transfer for ultra high-resolution images")

parser.add_argument('-s', '--style', type=str, help='path to style image', default = 'texture_images/texture_0016.png')
...
parser.add_argument('--iterations_mode', choices=['spst-baseline', 'spst-fast', 'debug'], default='spst-baseline', help='choice for the number of L-BFGS iterations at each scale: spst-baseline: long but best results, spst-fast: fewer iterations on large scales, debug: only 10 iterations at each scale to check installation/memory usage')
parser.add_argument('--device','-d', type=str, default='0', help='device number 0,1,... for cuda:0, cuda:1')
parser.add_argument('--minimal_scale_threshold', default = 256, type=int, help='size threshold to stop dividing by 2 when fixing the number of scales. 1023 (default): minimal scale is between 512 and 1023; 2047: minimal scale is between 1024 and 2047; 4095: minimal scale is between 2048 and 4095')
...
content_weights = [0.0] # use 0. content weight for texture synthesis
...
print("Original style image has size: ", style_img_pil_hr.size[1],' x ', style_img_pil_hr.size[0], '(no rescale for texture synthesis)')

# determine number of scales
wco, hco = style_img_pil_hr.size
...
content_base_string = 'texture_synthesis'
...
# write input image:
style_img_pil_hr.save(os.path.join(result_dir, style_base_string+'.png'))
...
for idx in range(nbscales):
  ...
  if style_img_pil.height*style_img_pil.width <= maxgpufit*1e6:
    opt_img_dev = device
  else:
    opt_img_dev = 'cpu'
  ...
  if idx == 0:
    #random init:
    bi,ci,hi,wi = style_image.shape
    opt_img = 10*torch.randn((bi,ci,hi,wi)).type_as(style_image.data)
    opt_img.requires_grad=True
    opt_img = opt_img.to(opt_img_dev)
  else:
    ...
    with torch.no_grad():
      ...
      opt_img_pil = opt_img_pil.resize(style_img_pil.size)
  ...
  # content layers unused since multiplied by 0 (but kept to minize changes in code):
  vgg_blocks_content_img = VGG_BY_BLOCKS(vgg, style_image, style_layers, content_layers = content_layers)
